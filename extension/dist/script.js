    ...
    statusToggle.addEventListener("change", (e) => {
        ...
        browser.tabs.query({ active: true, currentWindow: true }).then(tabs => {
            ...
            if (!tabs[0]?.url?.toLowerCase().includes("youtube.com/shorts")) {
                ...
                browser.storage.local.set({ applicationIsOn: isChecked }); // Still allow setting it
            }
            else {
                errMsg.innerText = ""; // Clear error on success
                browser.storage.local.set({ applicationIsOn: isChecked });
            }
        });
    });
...
function handleShortcutInputChange(element, storageKey, defaultValue) {
    return () => {
        ...
        if (!value.length) {
            ...
            // browser.storage.local.set({ [storageKey]: defaultValue.split('+') });
